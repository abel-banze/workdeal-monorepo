...
export const metadata = {
  title: "Histórico de verificações | Workdeal Admin",
};

const HISTORY_STATUSES = ["approved", "rejected", "in_review", "pending"] as const;
...
export default async function VerificationHistoryPage({
  ...
  const status = sp.status && (HISTORY_STATUSES as readonly string[]).includes(sp.status) ? sp.status : "approved";
  ...
  const buildHref = (overrides: { status?: string; page?: string }) => {
    ...
    return `/dashboard/verifications/history${s ? `?${s}` : ""}`;
  };

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold">Histórico de verificações</h1>
        <p className="text-sm text-muted-foreground">
          Pedidos já decididos por admin/moderator, com data e nota da revisão.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Decididos ({total})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <form method="GET" action="/dashboard/verifications/history" className="flex flex-wrap items-end gap-2">
          ...
          <VerificationsTable rows={rows} mode="history" />
