...
export async function placesAutocompleteAction(input: string): Promise<PlacesSuggestionsResult> {
  ...
  try {
    ...
    const res = token
      ? await apiFetchWithAuth<PlaceSuggestion[]>(`/api/v1/places/autocomplete?${qs.toString()}`, token, {
          cache: "no-store",
        })
      : await apiFetch<PlaceSuggestion[]>(`/api/v1/places/autocomplete?${qs.toString()}`, {
          cache: "no-store",
        });
...
export async function placesDetailsAction(placeId: string): Promise<PlaceDetailsResult> {
  ...
  try {
    const res = token
      ? await apiFetchWithAuth<PlaceDetails>(`/api/v1/places/details/${encodeURIComponent(placeId)}`, token, {
          cache: "no-store",
        })
      : await apiFetch<PlaceDetails>(`/api/v1/places/details/${encodeURIComponent(placeId)}`, {
          cache: "no-store",
        });
