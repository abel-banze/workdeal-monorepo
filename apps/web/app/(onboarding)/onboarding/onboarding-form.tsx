...
import { Phone, Clock, Check, X, Info, ArrowRight, ArrowLeft, ChevronDown, PartyPopper } from "lucide-react";

type Category = { id: string; name: string; slug: string };

const DRAFT_KEY = "wd:onb:draft:v1";

const PROVINCES = ["Cabo Delgado", "Cidade de Maputo", "Gaza", "Inhambane", "Manica", "Maputo", "Nampula", "Niassa", "Sofala", "Tete", "Zambézia"] as const;
const LEGAL_FORMS = [
...
const inputCls =
  "w-full rounded-lg border border-border bg-muted px-3 py-2 text-[13px] leading-none text-foreground placeholder:text-muted-foreground outline-none transition focus:border-primary focus:bg-card focus:ring-2 focus:ring-ring/50";
const textareaCls =
  "w-full rounded-lg border border-border bg-muted px-3 py-2 text-[13px] text-foreground placeholder:text-muted-foreground outline-none transition focus:border-primary focus:bg-card focus:ring-2 focus:ring-ring/50";
const selectCls =
  "w-full rounded-lg border border-border bg-muted px-3 py-2 text-[13px] text-foreground outline-none transition focus:border-primary focus:bg-card focus:ring-2 focus:ring-ring/50";
const labelCls = "text-sm font-medium";
const errorCls = "rounded-xl border border-destructive/20 bg-destructive/10 px-3.5 py-2.5 text-sm font-medium text-destructive";
const fieldErrCls = "text-xs font-medium text-destructive";
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.9fr_1.25fr] lg:items-start">
      ...
      <div className="lg:hidden">
        <div className="rounded-xl border border-border bg-muted px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-muted-foreground">{Math.round(((Math.min(activeStep + 1, steps.length)) / steps.length) * 100)}%</span>
          </div>
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted">
            <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${((Math.min(activeStep + 1, steps.length)) / steps.length) * 100}%` }} />
          </div>
          <ol className="mt-3 flex gap-2" aria-label="Progresso do onboarding">
            {steps.map((s, i) => {
              ...
              return (
                <li
                  ...
                  aria-label={`${s.title}${isDone ? " (concluído)" : ""}`}
                  className={`flex h-7 flex-1 items-center justify-center rounded-full border text-[11px] font-bold ${isDone ? "border-primary bg-primary text-primary-foreground" : isActive ? "border-primary bg-card text-primary" : "border-border bg-card text-muted-foreground"}`}
                >
        ...
        <h1 className="mt-4 text-[22px] font-bold leading-[0.95] tracking-tight text-foreground">
          Vamos criar o teu perfil
        </h1>
        <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">Completo, verificado e pronto para ser encontrado.</p>
      </div>
      ...
      <div className="hidden lg:block lg:sticky lg:top-[88px]">
        <div className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3.5 py-1.5 text-[11px] font-bold tracking-wide text-muted-foreground shadow-sm">
          <span className="size-1.5 rounded-full bg-primary animate-pulse" />
          CONFIGURAÇÃO INICIAL • ≈ 5 MINUTOS
        </div>
        <h1 className="mt-4 text-[30px] font-bold leading-[0.9] tracking-tight sm:text-[34px]">
          Vamos criar
          <br />
          o teu perfil
          <br />
          <span className="font-light text-muted-foreground">no ecossistema.</span>
        </h1>
        ...
        <div className="mt-6 rounded-xl border border-border bg-muted p-4">
          <p className="text-[11px] font-bold tracking-wide text-primary">ONBOARDING</p>
          <p className="mt-1 text-xs text-muted-foreground">4 passos • ~5 min</p>
          <ol className="relative mt-4 space-y-0" aria-label="Progresso do onboarding">
            <div className="absolute left-[15px] top-[14px] bottom-[14px] w-px bg-border" aria-hidden />
            {steps.map((s, i) => {
              const isActive = i === activeStep;
              const isDone = i < activeStep;
              return (
                <li key={s.title} aria-current={isActive ? "step" : undefined} className="relative flex gap-3 py-3">
                  <div
                    className={`flex size-8 shrink-0 items-center justify-center rounded-full border text-xs font-bold transition-colors ${
                      isDone ? "border-primary bg-primary text-primary-foreground" : isActive ? "border-primary bg-card text-primary shadow" : "border-border bg-card text-muted-foreground"
                    }`}
                  >
                    {isDone ? <Check className="size-3.5" aria-hidden /> : i + 1}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className={`text-sm font-bold leading-none ${isActive ? "text-foreground" : isDone ? "text-primary" : "text-muted-foreground"}`}>{s.title}</p>
                    <p className="text-xs text-muted-foreground">{s.desc}</p>
                    {i === 1 && (whatsappVerifiedAt || phoneVerifiedAt || emailVerifiedAt) && (
                      <p className="mt-1 inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-medium text-primary"><Check className="size-3" aria-hidden /> contacto verificado</p>
                    )}
                  </div>
                </li>
              );
          ...
          {previewSize && (
            <div className="mt-4 rounded-xl border border-primary/15 bg-card px-3 py-2 text-xs">
              <span className="font-bold text-primary">{previewSize.label}</span> <span className="text-muted-foreground">— {previewSize.desc}</span>
            </div>
          )}
        </div>

        <div className="mt-6 hidden items-center gap-2 text-[11px] font-bold tracking-wide text-muted-foreground lg:flex">
          <span>GLOBAL</span>
      ...
      <form
        ...
        noValidate
        className="rounded-xl bg-card p-6 shadow-sm ring-1 ring-foreground/10 sm:p-7"
      >
        <div className="mb-6 flex items-center justify-between gap-2">
          <p className="text-[11px] font-bold tracking-wide text-primary">ONBOARDING</p>
          <div className="flex items-center gap-2">
            <span className="rounded-full border border-border bg-muted px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">Olá, {userName.split(" ")[0]}</span>
            {activeStep < 3 && (
              <button
                type="button"
                onClick={() => router.push("/")}
                className="rounded-full px-2.5 py-1 text-[11px] font-medium text-muted-foreground transition hover:text-foreground hover:underline"
              >
                Guardar e sair
              </button>
            )}
        ...
        {restoredStep != null && activeStep < 3 && (
          <div className="mb-4 flex items-center justify-between gap-2 rounded-xl border border-primary/20 bg-primary/10 px-3.5 py-2.5 text-sm text-primary">
            <span>Bem-vindo de volta — continuaste no passo {restoredStep + 1} de 4.</span>
            <button type="button" onClick={() => setRestoredStep(null)} className="shrink-0 font-medium hover:underline">
              Dispensar
            </button>
          </div>
        )}
        ...
        {activeStep === 0 && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Como a empresa aparece no ecossistema. O nome aqui cria a organização e o perfil público.</p>
            </div>
            ...
            <div className="space-y-2 rounded-xl border border-border bg-muted p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full border border-border bg-card px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">Opcional</span>
              </div>
              <div className="relative">
                ...
                {placeStatus === "loading" && (
                  <span
                    className="absolute right-3 top-1/2 size-3 -translate-y-1/2 animate-spin rounded-full border-2 border-foreground/20 border-t-primary"
                    aria-hidden="true"
                  />
                )}
                {(placeSuggestions.length > 0 || placeStatus !== "idle") && (
                  <div className="absolute inset-x-0 top-full z-30 mt-1 overflow-hidden rounded-xl border border-border bg-card shadow-md">
                    {placeSuggestions.length > 0 ? (
                      placeSuggestions.map((s) => (
                        ...
                        >
                          <span className="block text-[13px] font-semibold text-foreground">{s.mainText}</span>
                          {s.secondaryText && <span className="block truncate text-xs text-muted-foreground">{s.secondaryText}</span>}
                        </button>
                      ))
                    ) : placeStatus === "loading" ? (
                      <div className="flex items-center gap-2 px-3 py-2.5">
                        <span
                          className="size-3 shrink-0 animate-spin rounded-full border-2 border-foreground/20 border-t-primary"
                          aria-hidden="true"
                        />
                        <span className="text-[13px] text-muted-foreground">A pesquisar no Google Maps…</span>
                      </div>
                    ) : placeStatus === "empty" ? (
                      <div className="px-3 py-2.5">
                        ...
                        <span className="mt-0.5 block text-xs text-muted-foreground">Sem problema — continua o preenchimento manual abaixo.</span>
                      </div>
                    ) : (
                      <div className="px-3 py-2.5 text-[13px] font-medium text-destructive">A pesquisa falhou — tenta novamente.</div>
                    )}
                  </div>
                )}
              </div>
              {pickedPlaceLabel && (
                <p className="inline-flex flex-wrap items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary">
                  <Check className="size-3.5 shrink-0" aria-hidden /> Dados pré-preenchidos de “{pickedPlaceLabel}” — revê e ajusta nos campos
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-4 rounded-xl border border-border bg-muted p-3">
                  <div className="relative size-16 shrink-0 overflow-hidden rounded-xl border border-border bg-card flex items-center justify-center">
                    {logoPreview || logoUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={logoPreview ?? logoUrl ?? ""} alt="Pré-visualização do logótipo" className="size-full object-cover" />
                    ) : (
                      <span className="text-[10px] font-bold tracking-widest text-muted-foreground">LOGO</span>
                    )}
                    {logoUploading && (
                      <span className="absolute inset-0 grid place-items-center bg-card/70">
                        <span className="size-4 animate-spin rounded-full border-2 border-foreground/20 border-t-primary" />
                      </span>
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap gap-2">
                      <label className="inline-flex cursor-pointer items-center justify-center rounded-full border border-border bg-card px-4 py-2 text-xs font-bold text-foreground transition hover:bg-muted has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-50">
                      ...
                      {(logoUrl || logoPreview) && (
                        <button type="button" onClick={clearLogo} disabled={logoUploading} className="rounded-full border border-destructive/20 bg-card px-4 py-2 text-xs font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50">
                          Remover
                        </button>
                      )}
                    </div>
                    <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">PNG, JPG ou WebP até 5 MB. Quadrado 512×512 recomendado. Opcional — podes adicionar depois.</p>
                  </div>
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">Define o porte da empresa e o selo correspondente.</p>
              </div>
              <div className="space-y-3">
                ...
                >
                  <ComboboxChips
                    ref={catAnchor}
                    className="min-h-[38px] rounded-lg border border-border bg-muted px-2 py-1 text-[13px] transition focus-within:border-primary focus-within:bg-card focus-within:ring-2 focus-within:ring-ring/50"
                  >
                    {selectedCats.map((id) => {
                      const cat = categories.find((c) => c.id === id);
                      return (
                        <ComboboxChip key={id} className="bg-primary text-primary-foreground hover:bg-primary/80">
                          {cat?.name ?? id}
                        </ComboboxChip>
                      );
                    })}
                  ...
                  <ComboboxContent anchor={catAnchor} className="z-50">
                    <ComboboxList>
                      {categories
                        ...
                        .slice(0, 30)
                        .map((c) => (
                          <ComboboxItem key={c.id} value={c.id} className="data-[selected]:bg-primary data-[selected]:text-primary-foreground">
                            {c.name}
                          </ComboboxItem>
                        ))}
                    </ComboboxList>
                    <ComboboxEmpty className="px-3 py-6 text-center text-sm text-muted-foreground">Nenhuma categoria encontrada.</ComboboxEmpty>
                  </ComboboxContent>
                </Combobox>
              ...
              {/* Dados adicionais — opcionais, editáveis depois no painel */}
              <Collapsible>
                <CollapsibleTrigger className="flex w-full items-center justify-between rounded-xl border border-border bg-muted px-4 py-3 text-left transition hover:bg-card">
                  <span>
                    <span className="block text-sm font-semibold">Dados da empresa (opcional)</span>
                    <span className="block text-xs text-muted-foreground">NUIT, forma jurídica, volumes, apresentação e código de indicação</span>
                  </span>
                  <ChevronDown className="size-4 shrink-0 transition-transform [[data-panel-open]&]:rotate-180" aria-hidden />
                </CollapsibleTrigger>
                ...
                <label htmlFor="onb-affiliateCode" className={labelCls}>
                  Código de indicação <span className="font-normal normal-case text-muted-foreground">(opcional)</span>
                </label>
                <input
                  ...
                  className={inputCls}
                />
                {affiliateStatus === "checking" && <p className="text-xs text-muted-foreground">A validar código…</p>}
                {affiliateStatus === "valid" && (
                  <p className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary"><Check className="size-3.5" aria-hidden /> Código válido — aplicado ao publicar</p>
                )}
                {affiliateStatus === "invalid" && (
                  <p className="text-xs font-medium text-destructive">Código inválido — verifica ou remove.</p>
                )}
                    <p className="text-xs leading-relaxed text-muted-foreground">Ouve falar da Workdeal por um parceiro? Adiciona o código dele e ele ganha ao veres a tua empresa a crescer.</p>
                  </div>
                </CollapsibleContent>
              </Collapsible>
            </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-primary px-6 py-3 text-sm font-bold text-primary-foreground hover:bg-primary/80"
              >
                Continuar para contactos <ArrowRight className="size-4" aria-hidden />
              </button>
            </div>
          </div>
        )}
        ...
        {activeStep === 1 && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Verifica pelo menos um contacto com código de 6 dígitos.</p>
            </div>

            {msg && (
              <div className={`flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-sm font-medium ${msg.type === "success" ? "border border-primary/20 bg-primary/10 text-primary" : msg.type === "error" ? "border border-destructive/20 bg-primary/10 text-destructive" : "border border-border bg-muted text-foreground"}`}>
                {msg.type === "success" && <Check className="size-4 shrink-0" aria-hidden />}
                {msg.type === "error" && <X className="size-4 shrink-0" aria-hidden />}
                {msg.type === "info" && <Info className="size-4 shrink-0" aria-hidden />}
                {msg.text}
              </div>
            )}
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-1.5 flex gap-2">
                <InputGroup className="h-11 flex-1 rounded-lg border-border bg-muted has-[[data-slot=input-group-control]:focus-visible]:border-primary has-[[data-slot=input-group-control]:focus-visible]:ring-2 has-[[data-slot=input-group-control]:focus-visible]:ring-ring/50 has-[[data-slot=input-group-control]:focus-visible]:bg-card">
                  <InputGroupAddon align="inline-start" className="border-r border-border pl-3">
                    <InputGroupText className="gap-1.5 text-[13px] font-semibold text-muted-foreground">
                      <Phone className="size-3.5" />
                      +258
                    </InputGroupText>
                  </InputGroupAddon>
                  <InputGroupInput id="onb-whatsapp" value={whatsapp} onChange={(e) => { setWhatsapp(e.target.value); if (whatsappOtp && !whatsappVerifiedAt) { setWhatsappOtp(null); setWhatsappInput(""); } }} placeholder="82 000 0001" aria-invalid={!!fieldErrors.whatsapp} className="h-full px-3 text-[13px] text-foreground placeholder:text-muted-foreground" />
                </InputGroup>
                <button type="button" onClick={() => sendOtp("whatsapp")} disabled={whatsappSending || !!whatsappVerifiedAt} className="shrink-0 rounded-lg border border-border bg-muted px-4 text-xs font-bold text-foreground transition hover:bg-card hover:border-primary disabled:opacity-40 disabled:hover:bg-muted disabled:hover:border-border">
                  {whatsappVerifiedAt ? <Check className="size-3.5" aria-hidden /> : whatsappSending ? (
                    <span className="flex items-center gap-1.5"><span className="size-3 animate-spin rounded-full border-2 border-foreground/20 border-t-foreground" /> Enviar</span>
                  ) : timers.whatsapp ? "Reenviar" : "Enviar código"}
                </button>
              </div>
              {whatsappVerifiedAt ? (
                <p className="mt-2.5 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                  <span className="size-1.5 rounded-full bg-primary" /> {verifiedLabel(whatsappVerifiedAt, "whatsapp")}
                </p>
              ) : whatsappOtp ? (
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <InputOTP maxLength={6} value={whatsappInput} onChange={setWhatsappInput} containerClassName="gap-1.5">
                      <InputOTPGroup className="gap-1.5">
                        {[0, 1, 2].map((i) => (
                          <InputOTPSlot key={i} index={i} className="size-10 rounded-lg border border-border bg-muted text-base font-semibold text-foreground data-[active=true]:border-primary data-[active=true]:ring-2 data-[active=true]:ring-ring/50 data-[active=true]:bg-card" />
                        ))}
                      </InputOTPGroup>
                    ...
                    <button type="button" onClick={() => verifyOtp("whatsapp")} disabled={whatsappInput.length < 6} className="shrink-0 rounded-lg bg-primary px-5 py-2.5 text-xs font-bold text-primary-foreground transition hover:bg-primary/80 disabled:opacity-40">
                      Verificar
                    </button>
                  </div>
                  {timers.whatsapp && (
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-1.5 flex gap-2">
                <InputGroup className="h-11 flex-1 rounded-lg border-border bg-muted has-[[data-slot=input-group-control]:focus-visible]:border-primary has-[[data-slot=input-group-control]:focus-visible]:ring-2 has-[[data-slot=input-group-control]:focus-visible]:ring-ring/50 has-[[data-slot=input-group-control]:focus-visible]:bg-card">
                  ...
                  <InputGroupInput id="onb-phone" value={phone} onChange={(e) => { setPhone(e.target.value); if (phoneOtp && !phoneVerifiedAt) { setPhoneOtp(null); setPhoneInput(""); } }} placeholder="84 000 0000" aria-invalid={!!fieldErrors.phone} className="h-full px-3 text-[13px] text-foreground placeholder:text-muted-foreground" />
                </InputGroup>
                <button type="button" onClick={() => sendOtp("phone")} disabled={phoneSending || !!phoneVerifiedAt} className="shrink-0 rounded-lg border border-border bg-muted px-4 text-xs font-bold text-foreground transition hover:bg-card hover:border-primary disabled:opacity-40 disabled:hover:bg-muted disabled:hover:border-border">
                  {phoneVerifiedAt ? <Check className="size-3.5" aria-hidden /> : phoneSending ? (
                    <span className="flex items-center gap-1.5"><span className="size-3 animate-spin rounded-full border-2 border-foreground/20 border-t-foreground" /> Enviar</span>
              ...
              {phoneVerifiedAt ? (
                <p className="mt-2.5 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                  <span className="size-1.5 rounded-full bg-primary" /> {verifiedLabel(phoneVerifiedAt, "phone")}
                </p>
              ) : phoneOtp ? (
                <div className="mt-3 space-y-2.5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <button type="button" onClick={() => verifyOtp("phone")} disabled={phoneInput.length < 6} className="shrink-0 rounded-lg bg-primary px-5 py-2.5 text-xs font-bold text-primary-foreground transition hover:bg-primary/80 disabled:opacity-40">
                      Verificar
                    </button>
                  </div>
                  {timers.phone && (
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-1.5 flex gap-2">
                <Input id="onb-email" type="email" value={email} onChange={(e) => { setEmail(e.target.value); if (emailOtp && !emailVerifiedAt) { setEmailOtp(null); setEmailInput(""); } }} placeholder="geral@empresa.co.mz" aria-invalid={!!fieldErrors.email} className="h-11 flex-1 rounded-lg border-border bg-muted px-3 text-[13px] text-foreground placeholder:text-muted-foreground focus:border-primary focus:bg-card focus:ring-2 focus:ring-ring/50" />
                <button type="button" onClick={() => sendOtp("email")} disabled={emailSending || !!emailVerifiedAt} className="shrink-0 rounded-lg border border-border bg-muted px-4 text-xs font-bold text-foreground transition hover:bg-card hover:border-primary disabled:opacity-40 disabled:hover:bg-muted disabled:hover:border-border">
                  {emailVerifiedAt ? <Check className="size-3.5" aria-hidden /> : emailSending ? (
                    <span className="flex items-center gap-1.5"><span className="size-3 animate-spin rounded-full border-2 border-foreground/20 border-t-foreground" /> Enviar</span>
              ...
              {emailVerifiedAt ? (
                <p className="mt-2.5 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                  <span className="size-1.5 rounded-full bg-primary" /> {verifiedLabel(emailVerifiedAt, "email")}
                </p>
              ) : emailOtp ? (
                <div className="mt-3 space-y-2.5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <button type="button" onClick={() => verifyOtp("email")} disabled={emailInput.length < 6} className="shrink-0 rounded-lg bg-primary px-5 py-2.5 text-xs font-bold text-primary-foreground transition hover:bg-primary/80 disabled:opacity-40">
                      Verificar
                    </button>
                  </div>
                  {timers.email && (
            ...
            <div className="flex justify-between">
              <button type="button" onClick={() => setActiveStep(0)} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-5 py-3 text-sm font-semibold text-foreground hover:bg-muted">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-primary px-6 py-3 text-sm font-bold text-primary-foreground hover:bg-primary/80"
              >
                Continuar para presença <ArrowRight className="size-4" aria-hidden />
              </button>
            </div>
          </div>
        )}
        ...
        {activeStep === 2 && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Onde a empresa aparece e é encontrada — localização e selos.</p>
            </div>
            ...
            <div className="rounded-xl border border-border bg-muted px-3 py-3">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {selectedCats.length === 0 ? (
                  <span className="text-xs text-muted-foreground">Nenhuma — volta ao passo 1 para escolher</span>
                ) : (
                  selectedCats.map((id) => {
                    const cat = categories.find((c) => c.id === id);
                    return (
                      <span key={id} className="inline-flex rounded-full bg-primary px-2.5 py-1 text-xs font-medium text-primary-foreground">
                        {cat?.name ?? id}
                      </span>
                    );
                  })
              ...
              <button type="button" onClick={() => setActiveStep(0)} className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-primary hover:underline">
                <ArrowLeft className="size-3.5" aria-hidden /> Editar categoria
              </button>
            </div>
            ...
            <div className="space-y-3 rounded-xl border border-border bg-muted p-4">
              ...
              {pickedPlaceLabel && (
                <p className="inline-flex flex-wrap items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary">
                  <Check className="size-3.5 shrink-0" aria-hidden /> Dados pré-preenchidos de “{pickedPlaceLabel}” — revê e ajusta abaixo
                  <button type="button" onClick={clearLocation} className="font-bold hover:underline">
                    limpar
                  </button>
                </p>
              ...
              {formattedAddress && (
                <p className="text-xs text-muted-foreground">
                  <span className="font-semibold text-foreground">Morada:</span> {formattedAddress}
                </p>
              )}
              <p className="text-xs text-muted-foreground">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                {tags.length > 0 ? (
                      ...
                      {tags.map((t) => {
                        ...
                        return (
                          <button
                            ...
                            className={
                              on
                                ? "rounded-full border border-primary bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground transition"
                                : "rounded-full border border-border bg-muted px-3 py-1.5 text-xs font-medium text-muted-foreground transition hover:border-primary/40 hover:text-primary disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-border disabled:hover:text-muted-foreground"
                            }
                          >
                            {t.name}
                          </button>
                  ...
                  <p className="text-xs text-muted-foreground">Sem tags disponíveis — podes adicioná-las depois no painel.</p>
                )}
              </div>
              <div className="space-y-1.5">
                ...
                {BUSINESS_HOURS_OPTIONS.map((o) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full border px-3 py-1.5 text-xs font-medium transition ${active ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground hover:bg-muted"}`}
                    >
                      {o}
                    </button>
                  );
            ...
            {error && <p role="alert" aria-live="polite" className={errorCls}>{error}</p>}
            <div className="flex justify-between">
              <button type="button" onClick={() => setActiveStep(1)} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-5 py-3 text-sm font-semibold text-foreground hover:bg-muted">
                <ArrowLeft className="size-4" aria-hidden /> Voltar
              </button>
              <button
                type="submit"
                disabled={loading || logoUploading}
                className="rounded-full bg-primary px-6 py-3 text-sm font-bold text-primary-foreground hover:bg-primary/80 disabled:opacity-50"
              >
        ...
        {activeStep === 3 && (
          <div className="space-y-4">
            <div className="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/10 p-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-foreground">Perfil de {profileName.trim() || "empresa"} criado</p>
                <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">
                  É assim que a empresa entra no ecossistema. Espreita como ficou antes de decidir sobre a verificação.
                </p>
                {organizationId && (
                  <Link href={`/dashboard/${organizationId}/profile`} className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-primary hover:underline">
                    Ver como ficou <ArrowRight className="size-3.5" aria-hidden />
                  </Link>
                )}
              </div>
            </div>
            ...
            <p className="text-sm text-muted-foreground">Selo “Verificado” desbloqueia confiança máxima. Podes saltar e pedir depois.</p>
            <div className="grid gap-3">
              <button
                type="button"
                aria-pressed={wantVerification === true}
                onClick={() => setWantVerification(true)}
                className={`rounded-xl border p-4 text-left transition ${wantVerification === true ? "border-primary bg-primary/5" : "border-border bg-card hover:bg-muted"}`}
              >
                <p className="text-sm font-bold text-foreground">Sim, pedir agora</p>
                <p className="text-xs text-muted-foreground">Recomendado — 24–48h úteis. Contactos já verificados dão prioridade.</p>
              ...
              <button
                type="button"
                aria-pressed={wantVerification === false}
                onClick={() => setWantVerification(false)}
                className={`rounded-xl border p-4 text-left transition ${wantVerification === false ? "border-primary bg-primary/5" : "border-border bg-card hover:bg-muted"}`}
              >
                <p className="text-sm font-bold text-foreground">Fazer depois</p>
                <p className="text-xs text-muted-foreground">Vais directo para o painel.</p>
              </button>
            </div>
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    <button
                      type="button"
                      aria-pressed={verifyLevel === "level1"}
                      onClick={() => setVerifyLevel("level1")}
                      className={`rounded-xl border p-3 text-left transition ${verifyLevel === "level1" ? "border-primary bg-primary/5" : "border-border bg-card"}`}
                    >
                      <span className="block text-sm font-bold text-foreground">1º grau — Verificado</span>
                      <span className="mt-0.5 block text-xs leading-snug text-muted-foreground">Todos os documentos de registo legal.</span>
                    </button>
                    <button
                      type="button"
                      aria-pressed={verifyLevel === "level2"}
                      onClick={() => setVerifyLevel("level2")}
                      className={`rounded-xl border p-3 text-left transition ${verifyLevel === "level2" ? "border-primary bg-primary/5" : "border-border bg-card"}`}
                    >
                      <span className="block text-sm font-bold text-foreground">2º grau — Em legalização</span>
                      <span className="mt-0.5 block text-xs leading-snug text-muted-foreground">Ainda em processo de legalização.</span>
                    </button>
                  </div>
                </div>
                <div className="space-y-1.5">
            ...
            {error && <p role="alert" aria-live="polite" className={errorCls}>{error}</p>}
            <div className="flex justify-between">
              <button type="button" onClick={() => setActiveStep(2)} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-5 py-3 text-sm font-semibold text-foreground hover:bg-muted">
                <ArrowLeft className="size-4" aria-hidden /> Voltar
              </button>
              <button
                type="button"
                ...
                className={`rounded-full px-6 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50 ${wantVerification ? "bg-primary hover:bg-primary/80" : "bg-primary hover:bg-primary/80"}`}
              >
                {loading ? "A processar…" : wantVerification ? "Pedir verificação" : "Ir para painel — sem verificação"}
              </button>
            </div>
          </div>
        )}
        </div>
        <p className="mt-6 text-center text-xs text-muted-foreground">Podes completar ou editar tudo depois no painel. Sem fidelização.</p>
      </form>
    </div>
  );
}
